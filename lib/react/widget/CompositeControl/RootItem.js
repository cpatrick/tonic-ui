        ...
        return (
            <div className='CompositePipeline_section'>
                        ...
                        <div
                            ...
                            { model.getColor(layer).map(function(color) {
                                return <div key={color}
                                            data-color={color}
                                            className={'CompositePipeline_item-colorBy_item ' + (model.isActiveColor(layer, color) ? ' is-selected': '')} >{ model.getColorToLabel(color) }
                                        </div>;
                            })}
                        </div>
                    </div>
                ...
                <div className={ (hasOpacity && !hasChildren) ? 'CompositePipeline_item' : 'is-hidden'}>
                    <input className='CompositePipeline_item-opacity' type='range' min='0' max='100' value={ model.getOpacity(layer) } onChange={ this.updateOpacity }/>
                </div>
